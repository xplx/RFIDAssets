...
@Service
public class ConnReadermplNewTest implements ApplicationContextAware {

	@Resource
	private NewRfidTagTest newRfidTagTest;
	...
	public volatile static Thread ReaderOutA;// 出库读写器
	...
	public void getMonitorWarn() {
		MonitorWarn = new Thread(new Runnable() {
			...
			public void run() {
				...
				while (true) {
					newRfidTagTest.updateRfidWarnInfo();// 监控进出库报警
	...
	public void getReaderIn(int serverPort) {
		ReaderIn = new Thread(new Runnable() {
			...
			public void run() {
				...
				while (true) {
					newRfidTagTest.getRfidInOut(serverPort);
					delay_ms(10000);// 10s休眠时间
				}
			}
		});
	...
	public void getReaderOut(int serverPort) {
		ReaderOut = new Thread(new Runnable() {
			...
			public void run() {
				log.info("TCP启动进出库线程");// run的方法体代表要执行的任务
				while (true) {
					newRfidTagTest.getRfidInOutA(serverPort);// 8082
					delay_ms(10000);// 10s休眠时间
				}
			}
		});
	...
	public void getReaderOutA(int serverPort) {
		ReaderOutA = new Thread(new Runnable() {
			int count = 0;
			...
			public void run() {
				log.info("TCP启动进出库线程");// run的方法体代表要执行的任务
				while (true) {
					newRfidTagTest.getRfidInOutB(serverPort);// 8082
					delay_ms(10000);// 10s休眠时间
				}
			}
		});
		if (ReaderOutA.isAlive() == false) {// 判断线程是否启动
			ReaderOutA.setDaemon(true);//
			ReaderOutA.start();//
			ReaderOutA.setPriority(4);
		}
...
//			getReaderOut(8083);// 监视盘点线程80838084
//			getReaderOutA(8084);
